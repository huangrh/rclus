...
fit_df <- data.frame(x=grp_score,cluster=fit$cluster,cen=fit$centers[fit$cluster])
sum((fit_df$x-fit_df$cen)^2)
```
...
# calculate the sum square of distance between each observation and its corresponding centers.
df = data.frame(x = grp_score,cluster = clusters,center = centers[clusters])
sum((df$x-df$center)^2)
```
